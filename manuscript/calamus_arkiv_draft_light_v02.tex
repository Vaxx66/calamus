...
Let $S$ be a measurable space and let $\Prb(S)$ be the set of the probability
...
We recall the definition of statistical model given by McCullagh
...
the compatibility which is needed in order to compare a model at two designs.
...
objects are the feature spaces and the morphisms are the linear feature maps.
...
$\catV \times \catD^{\op} \to \Stoch$. The morphisms $\mathbf{L}(m)$ and
$\mathbf{R}(m)$ are Dirac kernels, hence deterministic, for every morphism
$m$. The components $\mathbf{P}_{v,d}$ need not be deterministic.
\end{theorem}
...
The measurable space $\Lambda(\omega)$ is the space of the values at $\omega$.
...
We denote by $\Alg_k$ the category of the associative unital $k$-algebras and
...
Hochschild cohomology groups as coefficients, and it is not used here.
\end{remark}
...
\]
be the class of the corrections which leave the component at the merged design
...
in the finite runs is the discretisation of this argument. Its two classes in
...
A Markov category \emph{has conditional
distributions}~\cite[Def.~11.1]{fritz_synthetic_2020} if for every state
$\psi \colon I \rightsquigarrow X \otimes Y$ there is a morphism
$\psi|_X \colon X \rightsquigarrow Y$ such that
\begin{equation}\label{eq:conditional}
\psi \;=\; (\id_X \otimes \psi|_X) \circ \mathrm{copy}_X \circ \psi_X ,
\end{equation}
where $\psi_X$ is the first marginal of $\psi$, i.e.\ its composition with the
discard map of $Y$. Cho and Jacobs call $\psi|_X$ a \emph{disintegration} of
$\psi$~\cite[Def.~3.5]{cho_disintegration_2019}. In $\Stoch$
equation~\eqref{eq:conditional} reads
\[
\psi(S \times T) \;=\; \int_{S} \psi|_X(T \mid x)\,\psi_X(\mathrm{d}x)
\qquad (S \in \Sigma_X,\ T \in \Sigma_Y),
\]
i.e.\ $\psi|_X$ is a regular conditional probability of $\psi$ given the first
coordinate~\cite[Ex.~11.3, eq.~(11.3)]{fritz_synthetic_2020},
\cite[Ex.~3.7]{cho_disintegration_2019}. Such a kernel need not exist for
arbitrary measurable spaces, and it exists when $Y$ is standard
...
Let $p \colon \Theta \rightsquigarrow X$ be a morphism. Two morphisms
$f,g \colon X \rightsquigarrow Y$ are called \emph{$p$-a.s.\ equal} if
$(\id_X \otimes f)\circ\mathrm{copy}_X\circ p =
(\id_X \otimes g)\circ\mathrm{copy}_X\circ p$~\cite[Def.~13.1]{fritz_synthetic_2020}.
In $\Stoch$ this means that $f(T\mid -)$ and $g(T \mid -)$ agree
$p(-\mid\theta)$-almost everywhere for every $T$ and every
$\theta$~\cite[Ex.~13.3]{fritz_synthetic_2020}. The conditional $\psi|_X$
of~\eqref{eq:conditional} is unique up to $\psi_X$-a.s.\
equality~\cite[Prop.~13.7]{fritz_synthetic_2020},
\cite[\S5]{cho_disintegration_2019}, and this is the sense in which the
posteriors of Sections~\ref{sec:bayes} and~\ref{sec:ridge} are unique.

Let $\sigma \colon I \rightsquigarrow X$ be a state and let
$c \colon X \rightsquigarrow Y$ be a morphism. The conditional with respect to
$Y$ of the joint state $(\id_X \otimes c) \circ \mathrm{copy}_X \circ \sigma$
is called the \emph{Bayesian inversion} of $c$ with respect to $\sigma$, and it
is denoted by $c^{\dagger}_{\sigma} \colon Y \rightsquigarrow X$. Thus it is a
morphism $d$ such that
\begin{equation}\label{eq:inversion}
(\id_X \otimes c)\circ\mathrm{copy}_X\circ\sigma
\;=\;
(d \otimes \id_Y)\circ\mathrm{copy}_Y\circ c\circ\sigma
\end{equation}
(see~\cite[eq.~(5)]{cho_disintegration_2019}
and~\cite[eq.~(13.2)]{fritz_synthetic_2020}). In $\Stoch$
equation~\eqref{eq:inversion} reads
\[
\int_{S} c(T\mid x)\,\sigma(\mathrm{d}x) \;=\;
\int_{T} d(S \mid y)\,(c\circ\sigma)(\mathrm{d}y)
\qquad (S \in \Sigma_X,\ T \in \Sigma_Y)
\]
\cite[Ex.~3.9]{cho_disintegration_2019}. Hence a Bayesian inversion exists in
$\BorelStoch$ and it is unique up to $(c\circ\sigma)$-a.s.\ equality. In
statistical terms, $\sigma$ is the prior, $c$ is the model, $c \circ \sigma$ is
the prior predictive distribution and $c^{\dagger}_{\sigma}$ is the posterior.
The posterior of Definition~\ref{def:prior} is $c^{\dagger}_{\sigma}$, where
$\sigma$ is the prior and $c$ is the component of the bridged model, and
Proposition~\ref{prop:ridge} computes it for the Gaussian linear model.
...
Section~\ref{sec:meaningful} play different roles for them.
...
in the one-way scheme the two selections
$i_{1},i_{2} \colon \{*\} \to \{1,2\}$ would give $i_{1} \circ x = i_{2} \circ x$,
which fails for $u \neq \emptyset$. With these conventions the naturality of
$\mathbf P$ at $\psi \colon \omega \to \omega'$ is the
square~\eqref{eq:natsquare} at $(\id_{u},\psi)$, i.e.
\begin{equation}\label{eq:natfibre}
\mathbf{R}(\psi) \circ \mathbf P_{\omega'}
\;=\; \mathbf P_{\omega} \circ \del\Theta_\Meas(\psi)
\qquad\text{in } \Stoch ,
\end{equation}
which is condition (N-$\catO$) of Section~\ref{sec:worked}. We keep the factor
$\mathbf{R}(\psi)$, which is an identity here, because in this form the proofs
below hold over $\catD$, where $\mathbf{R}$ acts by marginalisation. The
quantifier classes
$\catO_{\mathrm{red}} \subseteq \catO_{\mathrm{inj}} \subseteq \catO$ are those
of Section~\ref{sec:meaningful}. In Example~\ref{ex:linreg} the morphisms of
$\catO_{\mathrm{inj}}$ are the injective linear maps.
...
\]
The \emph{posterior} is the conditional of the Bayesian model with respect to
its second output, i.e.\ its disintegration in the sense
of~\cite[Def.~3.5]{cho_disintegration_2019}
and~\cite[Def.~11.1]{fritz_synthetic_2020}. It is the Bayesian inversion
$\mathbf P_{\omega}^{\dagger} \colon \mathbf{R}(\omega) \rightsquigarrow
\Theta_\Meas(\omega)$ of $\mathbf P_{\omega}$ with respect to
$\pi_{\omega}$~\cite[eq.~(5)]{cho_disintegration_2019},
\cite[eq.~(13.2)]{fritz_synthetic_2020}, which exists in $\BorelStoch$ and is
unique up to $m_{\omega}$-a.s.\
equality~\cite[Prop.~13.7]{fritz_synthetic_2020}.
...
which is disjoint from $\operatorname{im}\Theta_\Meas(\psi)$. In the finite
...
$\Theta_\Meas(\omega)$ to the states on $\mathbf{R}(\omega)$ is injective. If
$m$ is natural over $\cat{Q}$ then $\pi$ is $\cat{Q}$-coherent. In $\FinStoch$
...
Consider the merge $p$ of the one-way scheme (Example~\ref{ex:oneway}), for
which $\Theta(p)$ is the diagonal. By part~(1), if~\eqref{eq:priorsquare}
holds at $p$ then $\pi_{\omega_2}$ is supported on the diagonal, so that the
prior for two groups carries no more information than the prior for one
group. For this reason~\eqref{eq:priorsquare} is imposed over
$\catO_{\mathrm{red}}$ or over $\catO_{\mathrm{inj}}$ and not over $\catO$.
Consider now part~(2) in the one-way scheme, with $\omega = \{1,\dots,n\}$ and
...
coincide. Finally, note that part~(4) requires identifiability in mixtures,
i.e.\ the injectivity of the induced map on the states, which is stronger than
the injectivity of $\theta \mapsto \mathbf P_{\omega}(\theta)$.
\end{remark}
...
gives
\[
\pi^{J}_{\omega}(d\mu, d\sigma^{2}) \;\propto\;
(\sigma^{2})^{-(|\omega|+2)/2}\,d\mu\,d\sigma^{2},
\]
...
$\pi(d\zeta)f(z\mid\zeta)$ for any prior $\pi$. These are failures of
...
inversion because
$(\id \otimes \mathbf{P}_{d}^{\dagger}) \circ \mathrm{copy} \circ m_{d}$ is the
...
with underlying map $Uq$. It is the counit by the triangle identities. The
